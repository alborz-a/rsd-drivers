...
const OrderScreen = ({ route }) => {
    ...
    const alertDestinationChanged = (previousDestination, currentDestination, order) => {
        return Alert.alert(
            ...
            t('OrderScreen.waypointActivityCompleted', {
                previousDestinationAddress: previousDestination.getAttribute('address'),
                currentDestinationAddress: currentDestination.getAttribute('address'),
            }),
            [
                {
    ...
    const sendOrderActivityUpdate = useCallback(
        async (activity, proof) => {
            ...
            isUpdatingActivity.current = true;
            setLoadingOverlayMessage(`${t('OrderScreen.updatingActivity')} ${activity._resolved_status ?? activity.status}`);
            ...
            try {
                ...
                toast.success(t('OrderScreen.orderStatusUpdatedTo', { status: activity._resolved_status ?? activity.status }));
        ...
        <YStack flex={1} bg='$background'>
            ...
            >
                ...
                <YStack py='$4'>
                    ...
                    <Separator />
                    <SectionInfoLine title={t('OrderScreen.proofOfDelivery')} value={order.getAttribute('pod_required') ? titleize(order.getAttribute('pod_method')) : 'N/A'} />
                    <Separator />
                    ...
                    <Separator />
                    <SectionInfoLine title={t('OrderScreen.dateScheduled')} value={order.getAttribute('scheduled_at') ? formatLocalizedDate(new Date(order.getAttribute('scheduled_at')), 'PP HH:mm') : '-'} />
                    <Separator />
                    <SectionInfoLine title={t('OrderScreen.dateDispatched')} value={order.getAttribute('dispatched_at') ? formatLocalizedDate(new Date(order.getAttribute('dispatched_at')), 'PP HH:mm') : '-'} />
